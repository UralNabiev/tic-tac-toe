...
class GameVC: UIViewController, CAAnimationDelegate {
    ...
    var xCase: [Int] = []
    var oCase: [Int] = []
    let winCaseColor: UIColor = #colorLiteral(red: 0, green: 0.6763487458, blue: 1, alpha: 1)
    ...
    override func viewDidLoad() {
        ...
        oIsPlayerImg.image = UIImage(named: "player_off")
        xIsPlayerImg.image = UIImage(named: "player_onn")
    ...
    @IBAction func arrowsTapped(_ sender: UIButton) {
        ...
        if xIsPlayerImg.image == UIImage(named: "player_onn") {
                if (sender.currentTitle ?? "").isEmpty {
                    sender.isEnabled = false
                }
            sender.setTitle("X", for: .normal)
            xCase.append(sender.tag)
            xIsPlayerImg.image = UIImage(named: "player_off")
            oIsPlayerImg.image = UIImage(named: "player_onn")
        } else if oIsPlayerImg.image == UIImage(named: "player_onn") {
            if (sender.currentTitle ?? "").isEmpty {
                sender.isEnabled = false
            }
